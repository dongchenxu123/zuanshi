...
import {dayTestHome, createcombination, detailFast} from '../../help/linkUrl';
        ...
        if (id == 0) {
           ...
           axios.post(getcampaign, {
            ...
            .then(function (response) {
               ...
               var Areas = []
               var specailAreas = []
               var data = [471,463,577,599,576,531]
               for(var k=0; k<AreaIdList.length; k++) {
                  if(data.indexOf(AreaIdList[k]) > -1) {
                      specailAreas.push(AreaIdList[k] + '')
                  } else {
  ...
  changeArea (id, value) {
    ...
    setClick () {
       ...
       this.field.validate((errors, values) => {
            ...
            var editList = step4Data.editList
           ...
           var campagin = {
            ...
            .then(function (response) {
               ...
               if(settings != null) {
                   ...
                   editList = {}
                   combinationData = []
                   self.props.commonData({step, combinationData, editList})
                   if(obj) {
    ...
    render () { 
        ...
        if(obj) {
            var url = '/fast/test/detail/'+obj.id+'/'+obj.title+'/'+obj.status
        }
        return (
            ...
            <div className="panel-footer" style={{overflow: 'hidden', backgroundColor: '#fff'}}>
                {
                    ...
                    ? <div style={{float: 'right'}}>
                        <Button type="primary" onClick={this.setClick.bind(this)}>修改计划</Button>
                      </div>
                    : <div style={{float: 'right'}}>
                        <Link to={obj ? url : createcombination}><Button type="normal" style={{marginRight: '15px'}}>上一步</Button></Link>
